...
public class Estacionamento {
	...
	static String descricaoMarca[] = {"BMW, Volkswagen, Ford, Mercedes Benz, Chevrolet, Fiat,"
			+ " Audi, Toyota, honda, hyundai"}
	...
	String codMarcaVeic[] = new String [10];
	String descricaoMarca[] = new String [10];
	...
	public void entrarVeiculo() {
		...
		do {
			...
			ativo = 'S';
			codEst = codEstChave;
			System.out.println("Digite a placa do veículo (FIM para encerrar)... ");
			placa = Main.leia.nextLine;
			if (placa.equalsIgnoreCase("FIM")) {
				Main();
			}
		    System.out.println("Digite a data da operação: ");
			dataOperacao = Main.leia.nextLine;
			
			System.out.println("Digite o tipo da operação: ");
			tipoOperacao = Main.leia.next.CharAt(0);
			...
			do {
				...
				indiceMarcaVeic = pesquisarMarcaVeic(codMarcaVeic);
				if (indiceMarcaVeic = -1) {
					System.out.println("Não encontrou a marca do veículo!");
				}else {
					System.out.println("A marca do veículo é" + Main.descricaoMarca[indiceMarcaVeic]);
				}
...
//				confirmacao = Main.Main.leia.next().charAt(0);
//				if (confirmacao == 'S') {
...
//				Main.Main.leia.nextLine();  // limpa buffer de memoria
...
//					sexoAux = Main.Main.leia.next().charAt(0);
